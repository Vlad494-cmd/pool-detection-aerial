...
Fine-tuned YOLO26 object detectors that localize swimming pools in aerial imagery, with a
systematic model-size comparison (n / s / m) and a model-assisted auto-labeling loop.

![Example detections](examples/val_batch0_pred.jpg)

## Results

All models fine-tuned at 768 px (Ultralytics, early stopping from 100 epochs), evaluated on a held-out split:
...
train/valid/test splits. Training data extended via model-assisted auto-labeling (~1,400 additional images),
with pseudo-labels generated by a trained model and reviewed before merging.
Images are not committed to this repo — pull the dataset from Roboflow and point `data.yaml` at it.

## Approach

1. Annotation on Roboflow; YOLO-format export; augmentation.
2. Model-assisted auto-labeling loop on unlabeled imagery to grow the training set.
3. Fine-tuning YOLO26 n/s/m from pretrained weights (768 px, batch 8-16, early stopping).
